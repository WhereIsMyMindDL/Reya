...
from settings import decimal_places, RETRY_COUNT, delay_wallets, delay_transactions
...
value_eth = ['0', '0']                                            # минимальное и максимальное кол-во ETH для свапов и ликвы, в ковычках ("90") можно указать процент
...
send_list = ''
EIP1559_CHAINS = ['Ethereum', 'Zora', 'Optimism', 'Manta', 'opBNB', 'Base']
...
class Account:
    ...
    def wait_balance(self, value, rpc, contract_address=None):
        x = 0
        self.w3 = Web3(Web3.HTTPProvider(RPC[rpc]))
        while x < 4:
...
def intro(wallets):
    ...
    print()
    print(f'| {Fore.LIGHTGREEN_EX}Reya_bomj_tyga{Fore.RESET} |'.center(100, '='))
    print('\n')
...
def get_tx_data_withABI(self, value=0):
    if self.ChainName in EIP1559_CHAINS:
